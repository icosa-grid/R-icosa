...
# create grid
expect_silent(b  <- hexagrid(4, sf=TRUE))
...
# on random faces
expect_silent(fac <- faces(b))
names(numb) <- sample(fac, length(numb))
...
expect_silent(rand<- rpsphere(50, output="polar"))

# locate
expect_silent(cells <- locate(b, rand))
...
names(dummy) <- fac[1:length(dummy)]
