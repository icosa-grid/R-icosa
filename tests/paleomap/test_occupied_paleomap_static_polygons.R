...
model <- chronosphere::fetch(src="paleomap",ser="model",
	datadir="/mnt/sky/Dropbox/Software/icosa/data/chronosphere/",
	verbose=FALSE)
...
for(i in 1:length(ages)){
	...
	if(update){
		...
		png(paste0("icosa/tests/results/occupied/paleomap_static_polygons/", ages[i], ".png"), width=2000, height=1000)
		...
		dev.off()
		# and save the output
		saveRDS(occ, paste0("icosa/tests/results/occupied/paleomap_static_polygons/", ages[i], ".rds"))
		# loop counter
		cat(i, "\r")
		flush.console()
	# otherwise: do a test on the results
	}else{
		reference <- readRDS(paste0("icosa/tests/results/occupied/paleomap_static_polygons/", ages[i], ".rds"))
