...
for(i in 1:length(ages)){
	...
	if(update){
		...
		png(paste0("icosa/tests/results/occupied/paleomap_paleocoastlines/", ages[i], ".png"), width=2000, height=1000)
		...
		# and save the output
		saveRDS(occ, paste0("icosa/tests/results/occupied/paleomap_paleocoastlines/", ages[i], ".rds"))

		# loop counter
		cat(i, "\r")
		flush.console()
	}else{
		reference <- readRDS(paste0("icosa/tests/results/occupied/paleomap_paleocoastlines/", ages[i], ".rds"))
