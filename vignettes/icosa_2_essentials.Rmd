...
The density of these points can be easilly visualized with a plot.
...
- The argument `deg` refers to the expected length of the edge length of grid in degrees. This is given as angular distance between two cell vertices - same as degrees longitudinally (or latitudinally on the equator), 1° =~111 km. This is used to select a `tessellation` vector, which is directly controlling the grid resolution: how the faces of icosahedron are split up. The higher the product of this vector, the higher the resolution - ie. the more cells there will be in the grid.  

- The argument `sf=TRUE` indicates that we want to create a representation of the grid that can be used for additional spatial calculations (using the `sf` package) for example, to do 2D (projection-based) plotting. Since this is not always needed, and can significantly decrease performance, you need to indicate this when you create the grid. 
...
The output of `locate()` is designed so it would be usable as a new column in the table of coordinates (optional):
...
To assess the density of points, we need to count the number of points in every cell - which is actually nothing else, but the tabulation of the number of times that a face appears. This we can easilly do with the `table()` function:
...
There is a number of ways to visualize this number on the map, the simplest is to use a dedicated method of `plot()`, that takes such a named entity and the grid (with an `sf` representation, as [above](#land-polygons)) and plots this with the features made available via `sf`:
...
... you can also put them on top. Just make sure to set `reset=FALSE`:
...
plot(tab, gr) # plot named vector/table
```


If you are interested in more tutorials, you can continue from [3: Grid types and resolutions](https://adamkocsis.github.io/icosa/articles/icosa_3_grids.html) on the [package's webpage](https://adamkocsis.github.io/icosa/).
